class Flex::FlexReportsController < ApplicationController
  ...
  # PATCH/PUT /flex/reports/1
  # PATCH/PUT /flex/reports/1.json
  def update
    respond_to do |format|
      if @flex_report.update(flex_report_params)
        format.html { redirect_to @flex_report, notice: 'Report was successfully updated.' }
        format.json { render :show, status: :ok, location: @flex_report }
      else
  ...
  # DELETE /flex/reports/1
  # DELETE /flex/reports/1.json
  def destroy
    @flex_report.destroy
    respond_to do |format|
      format.html { redirect_to flex_reports_url, notice: 'Report was successfully destroyed.' }
