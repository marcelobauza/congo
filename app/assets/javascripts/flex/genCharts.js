...
$.each(charts, function (i, e) {
  if (e['title'] == 'Cantidad') {
    ...
    $.each(e['series'], function (index, series) {
      var  dataChart  = [];
      var labelsChart = [];

      $.each(series['data'], function(id, data) {
        labelg.push(data['name'])
        labelsChart.push(data['name'])
        dataChart.push(data['count'])
      })

      datasets.push({ label: labelsChart,
        type: 'line',
        data: dataChart,
        fill: false,
