class FutureProject < ApplicationRecord
  ...
  def save_future_project_data(geom, data, year, bimester, future_type)
    ...
    self.floors                 = data["N_PISOS"].to_i unless data["N_PISOS"]
    self.undergrounds           = data["SUBT"].to_i unless data["SUBT"]
    self.total_units            = data["T_UNID"].to_i unless data["T_UNID"]
    self.total_parking          = data["T_EST"].to_i unless data["T_EST"]
    self.total_commercials      = data["T_LOC"].to_i unless data["T_LOC"]
    self.m2_approved            = data["M2_APROB"] unless data["M2_APROB"]
    self.m2_built               = data["M2_EDIF"]
    self.m2_field               = data["M2_TERR"] unless data["M2_TERR"]
    self.t_ofi                  = data["T_OFI"] unless data["T_OFI"]
